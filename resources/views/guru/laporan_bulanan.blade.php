...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <span class="text-xs font-bold text-slate-400">Arsip Periodik</span>
    </div>
    ...
    <div class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-slate-50 bg-slate-50/50 flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-base font-black text-slate-800 tracking-tight">Laporan Rekap Bulanan Kelas</h2>
                <p class="text-[11px] text-slate-400">Saring dan tinjau seluruh rekaman kehadiran anak didik berdasarkan bulan operasional efektif KBM.</p>
            </div>
        </div>
        ...
        <div class="p-6 bg-slate-50/30">
            <form method="GET" class="max-w-md">
                <div class="space-y-1.5">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Pilih Periode Bulan</label>
                    <div class="relative">
        ...
        <div class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
            <div class="p-4 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                <div class="flex items-center space-x-2">
                    <span class="text-slate-500 text-xs"><i class="fa-solid fa-table-list"></i></span>
                    <h3 class="text-xs font-black text-slate-700 uppercase tracking-wider">Lembar Jurnal Bulanan</h3>
                </div>
                <span class="text-[10px] font-black text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-100/40">
                    Total Jurnal: {{ count($data) }} Baris
                </span>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                            <th class="py-3.5 px-6" style="width: 45%">Nama Lengkap Siswa</th>
                            <th class="py-3.5 px-6 text-center" style="width: 30%">Tanggal KBM</th>
                            <th class="py-3.5 px-6 text-right" style="width: 25%">Status</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 text-xs sm:text-sm text-slate-600 font-medium">
                        @foreach($data as $d)
                            ...
                            <tr class="hover:bg-slate-50/30 transition-colors duration-150 {{ $st == 'alpha' ? 'bg-rose-50/10' : '' }}">
                                ...
                                <td class="py-3.5 px-6 text-center text-slate-500 font-mono font-bold">
    ...
    @elseif(request('bulan'))
        <div class="bg-white border border-slate-100 rounded-2xl p-12 text-center space-y-2 shadow-sm">
            <div class="text-slate-300 text-4xl"><i class="fa-solid fa-folder-open"></i></div>
            <p class="text-slate-400 font-bold text-sm">Arsip Kosong</p>
            <p class="text-slate-400 text-xs">Belum ada rekaman log absensi kelas yang tercatat pada bulan ini.</p>
        </div>
    @endif
